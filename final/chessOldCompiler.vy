...
def timeout(player: int128) -> bool:
    if as_unitless_number(block.timestamp) > self.brd.lastmove + self.moveTime \
...
def enactMove(here: coord, there: coord, promotion: uint256, ep: bool):
    ...
    temp: int128 = self.brd.tiles[convert(here.col,int128)][convert(here.row,int128)] 
    self.brd.tiles[convert(there.col,int128)][convert(there.row,int128)] = temp 
    
    # remove piece from current tile
    self.brd.tiles[convert(here.col,int128)][convert(here.row,int128)] = 0
    ...
    if ep:
        ...
        self.brd.tiles[convert(there.col,int128)][convert(there.row,int128) + player * -1] = 0
    
    # if promotion, promote
    if (there.row == 7 or there.row == 0) \
            and (self.brd.tiles[convert(here.col,int128)][convert(here.row,int128)] == 1 or \
            self.brd.tiles[convert(here.col,int128)][convert(here.row,int128)] == -1):
        self.brd.tiles[convert(there.col,int128)][convert(there.row,int128)] = convert(promotion,int128) * player
...
def houseVic():
    ...
    if not (self.brd.moves > 100 or \
            as_unitless_number(block.timestamp) > self.brd.lastmove + self.moveTime * 10):
        assert(1 == 0)
